...
async function renderCommunityView(uuid) {
    ...
    if (!channels) {
        ...
        // Si ya está en caché, quizás queramos refrescar el rol si es necesario, pero por ahora asumimos que está bien o que loadChannels lo devuelve
        // Para ser consistentes con la lógica original, loadChannels siempre devuelve el rol
        // Haremos un fetch rápido si necesitamos el rol actualizado, pero ChannelManager.getCachedChannels solo devuelve canales
        // En la refactorización, loadChannels maneja todo.
        // Si está en caché, usamos lo que hay, si no, fetch.
    }
    
    // Nota: getCachedChannels solo devuelve el array. Si queremos el rol, deberíamos guardarlo también.
    // Para simplificar y asegurar datos frescos al entrar, llamaremos a loadChannels si no tenemos datos completos.
    // O podemos confiar en el item.role que ya tenemos del listado principal.
    ...
    if (window.ACTIVE_CHAT_UUID) {
        ...
        if (itemData) {
            ...
            if (itemData.type === 'community') {
                 ...
                 // Usar default_channel_uuid si existe
